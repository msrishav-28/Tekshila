...
const API_CONFIG = {
    ...
    endpoints: {
        ...
        analyzeQuality: '/api/analyze-quality',
        connectGithub: '/api/github/connect',
        getRepos: '/api/github/repos',
        getBranches: '/api/github/branches',
...
class FileUploadManager {
    ...
    init() {
        ...
        purposeRadios.forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                this.updateUIForPurpose(e.target.value);
            });
        });
    }
    ...
    updateUIForPurpose(purpose) {
        const projectNameGroup = document.querySelector('#projectName').closest('.input-group');
        const projectNameInput = document.getElementById('projectName');
        const generateBtn = document.getElementById('generateBtn');
        const uploadZoneText = document.querySelector('#uploadZone h3');
        const uploadZoneDescription = document.querySelector('#uploadZone p');
        
        if (purpose === 'readme') {
            // README mode
            projectNameGroup.style.display = 'block';
            projectNameInput.required = true;
            generateBtn.innerHTML = '<i class="fas fa-file-alt"></i><span>Generate README</span>';
            uploadZoneText.textContent = 'Drop files here or click to browse';
            uploadZoneDescription.textContent = 'Upload your project files to generate comprehensive README documentation';
        } else {
            // Comments mode
            projectNameGroup.style.display = 'block'; // Keep visible but not required
            projectNameInput.required = false;
            projectNameInput.placeholder = 'Project name (optional)';
            generateBtn.innerHTML = '<i class="fas fa-comment-dots"></i><span>Add Comments</span>';
            uploadZoneText.textContent = 'Drop code files here or click to browse';
            uploadZoneDescription.textContent = 'Upload your code files to add AI-generated comments and documentation';
        }
    }
}
...
class DocumentationManager {
    ...
    async generateDocumentation() {
        ...
        const loadingText = this.appState.currentPurpose === 'readme' 
            ? 'Generating README documentation...' 
            : 'Adding AI-generated comments to your code...';
        const loadingSubtext = this.appState.currentPurpose === 'readme'
            ? 'AI is analyzing your code and creating comprehensive documentation'
            : 'AI is analyzing your code structure and adding detailed comments';
        ...
        try {
            ...
            const response = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.generateDocs}`, {
                ...
                body: JSON.stringify({
                    ...
                    purpose: this.appState.currentPurpose,
            ...
            this.displayPreview(result.content, this.appState.currentPurpose);
            
            document.getElementById('downloadBtn').disabled = false;
            document.getElementById('copyBtn').disabled = false;
            
            const successMessage = this.appState.currentPurpose === 'readme'
                ? 'README documentation generated successfully!'
                : 'Code comments added successfully!';
            Utils.showToast(successMessage, 'success');

        } catch (error) {
            console.error('Error generating documentation:', error);
            const errorMessage = this.appState.currentPurpose === 'readme'
                ? `Failed to generate README: ${error.message}`
                : `Failed to add comments: ${error.message}`;
...
// Additional commented code would appear here...`;
    ...
    displayPreview(content, purpose) {
        ...
        if (purpose === 'readme') {
            ...
            // For comments, show code with syntax highlighting if possible
            const escapedContent = this.escapeHtml(content);
            previewContainer.innerHTML = `
                <div class="code-preview">
                    <div class="code-header">
                        <h4>Commented Code</h4>
                        <span class="code-info">AI-generated comments have been added to your code</span>
                    </div>
    ...
    downloadDocumentation() {
        ...
        const downloadMessage = this.appState.currentPurpose === 'readme'
            ? 'README file downloaded successfully!'
            : 'Commented code file downloaded successfully!';
        Utils.showToast(downloadMessage, 'success');
    }
...
class GitHubManager {
    ...
    async connectToGitHub() {
        ...
        try {
            ...
            this.appState.updateState('githubSessionId', result.session_id);
            ...
            Utils.showToast(`Failed to connect to GitHub: ${error.message}`, 'error');
        } finally {
            Utils.hideLoading();
        }
    }
    ...
    async loadRepositories() {
        try {
            const response = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.getRepos}?session_id=${this.appState.githubSessionId}`);
            
            if (!response.ok) {
                const error = await response.json();
                throw new Error(error.detail || 'Failed to load repositories');
            }
            ...
            Utils.showToast(`Failed to load repositories: ${error.message}`, 'error');
        }
    }
    ...
    async loadBranches() {
        ...
        try {
            const response = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.getBranches}?session_id=${this.appState.githubSessionId}&repo=${encodeURIComponent(selectedRepo)}`);
            ...
            Utils.showToast(`Failed to load branches: ${error.message}`, 'error');
        }
    }
    ...
    async createPullRequest() {
        ...
        try {
            ...
            const response = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.createPR}`, {
                ...
                body: JSON.stringify({
                    token: this.appState.githubToken,
                    repo: this.appState.selectedRepo,
            ...
            Utils.showToast(`Pull request created successfully! <a href="${result.pr_url}" target="_blank" style="color: var(--primary-color); text-decoration: underline;">View PR</a>`, 'success', 8000);
...
class QualityAnalysisManager {
    ...
    async analyzeCode() {
        ...
        try {
            // Read file content
            const content = await this.readFileContent(this.appState.qualityFile);
            ...
            const response = await fetch(`${API_CONFIG.baseUrl}${API_CONFIG.endpoints.analyzeQuality}`, {
                ...
                body: JSON.stringify({
                    filename: this.appState.qualityFile.name,
                    content: content
                })
            });
            ...
            Utils.showToast(`Failed to analyze code: ${error.message}`, 'error');
        } finally {
            Utils.hideLoading();
        }
    }
...
class TekshilaApp {
    ...
    setDefaultValues() {
        ...
        // Initialize UI for default purpose
        if (this.fileUploadManager) {
            this.fileUploadManager.updateUIForPurpose(this.appState.currentPurpose);
        }
    }
}
